...
from eao.serialization import to_json
...
############################################## assets with pv & wind matching load, only storage scales
print('generate std portfolio with all scaling assets')
pv_profile_matched      = {'start': df_profiles.index.values, 'values': df_profiles.SolarPower.values*scale_res['pv']}
onshore_profile_matched = {'start': df_profiles.index.values, 'values': df_profiles.OnshoreWindPower.values*scale_res['onshore']}

pv_vol_match       = Contract(name = 'pv',      min_cap= pv_profile_matched,      max_cap= pv_profile_matched,      nodes = node_green)
onshore_vol_match  = Contract(name = 'onshore', min_cap= onshore_profile_matched, max_cap= onshore_profile_matched, nodes = node_green)

print('write volume matched assets to file')
portf = Portfolio([downstream_contract,
                   spot_market,
                   pv_vol_match,
                   onshore_vol_match,
                   storage,
                   green_sales])
to_json(portf, file_vol_matching_assets)


############################################## fully scaling all assets
print('fully scaled portfolio')
portf = Portfolio([downstream_contract,
                   spot_market,
                   pv,
                   onshore,
                   storage,
                   green_sales])
to_json(portf, file_scaling_assets)
